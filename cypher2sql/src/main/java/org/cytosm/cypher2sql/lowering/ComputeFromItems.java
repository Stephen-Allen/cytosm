...
public class ComputeFromItems {
    ...
    private static class SQLTreeVisitor extends Walk.BaseSQLNodeVisitor {
        ...
        public void visitWithSelect(WithSelect withSelect) throws Cypher2SqlException {
            ...
            for (Var var : usedVars) {
                if (var instanceof NodeOrRelVar && !whereToGetTheVar.containsKey(var.uniqueName)) {
                    FromItem fromItem = new FromItem();
                    fromItem.variables.add(var);
            ...
            //
            for (Var v : usedVars) {
                if (v instanceof AliasVar && !whereToGetTheVar.containsKey(v.uniqueName)) {
                    AliasVar aliasVar = (AliasVar) v;
            ...
            // this WithSelect now:
            for (Var var : usedVars) {
                whereToGetTheVar.put(var.uniqueName, withSelect);
            }
        ...
        public void visitScopeSelect(ScopeSelect scopeSelect) throws Cypher2SqlException {
            ...
            for (Pair<WithSelect, Boolean> x : list) {
                if (!x.getValue()) {
